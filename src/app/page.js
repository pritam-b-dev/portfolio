...
export default async function Home() {
  ...
  return (
    <main className="overflow-x-hidden">
      ...
      {/* Projects */}
      <section id="projects" className="container mx-auto px-6 py-28 max-w-4xl">
        <p
          className="text-xs font-semibold tracking-[0.3em] uppercase mb-4"
          style={{ color: "var(--accent)" }}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {others.map((project) => (
            <div
              key={project._id}
              className="rounded-2xl border overflow-hidden transition-all duration-300 hover:-translate-y-1 group cursor-pointer"
              style={{
                background: "var(--card-bg)",
                borderColor: "var(--card-border)",
              }}
            >
              ...
              <div className="p-5">
                <h3 className="font-black mb-2">{project.name}</h3>
                <p
